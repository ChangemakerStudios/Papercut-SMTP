...
public class VelopackBridgeLogger(ILogger logger) : IVelopackLogger
{
    ...
    {
        logger.ForContext<VelopackApp>().Write(ToLogEventLevel(logLevel), exception, message ?? "[Empty]");   
    }

    public static LogEventLevel ToLogEventLevel(VelopackLogLevel velopackLevel)
    {
        return velopackLevel switch
        {
            ...
            _ => LogEventLevel.Information // fallback
        };
    }
}